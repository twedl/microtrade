...
The authoritative schema lives in an Excel workbook with one sheet per trade
type. Convert it to versioned YAML specs under `src/microtrade/specs/`:

```sh
uv run microtrade import-spec path/to/schema_workbook.xlsx \
...
Specs under `src/microtrade/specs/` should be generated by running
`microtrade import-spec` against the real schema workbook.
